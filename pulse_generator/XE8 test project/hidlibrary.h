...
template<typename T> string HIDLibrary<T>::GetDevicePath(HDEVINFO hInfoSet, PSP_DEVICE_INTERFACE_DATA oInterface)
{
		...
		if (!SetupDiGetDeviceInterfaceDetail(hInfoSet, oInterface, 0, 0, &nRequiredSize, 0))
		{
		   //	PSP_DEVICE_INTERFACE_DETAIL_DATA oDetail = (PSP_DEVICE_INTERFACE_DETAIL_DATA)malloc(nRequiredSize);
			PSP_DEVICE_INTERFACE_DETAIL_DATA_A  oDetail= (PSP_DEVICE_INTERFACE_DETAIL_DATA_A)malloc(nRequiredSize);
//			oDetail->cbSize = sizeof(SP_DEVICE_INTERFACE_DETAIL_DATA);
			oDetail->cbSize = sizeof(PSP_DEVICE_INTERFACE_DETAIL_DATA_A);

			if (SetupDiGetDeviceInterfaceDetailA(hInfoSet, oInterface, oDetail, nRequiredSize, &nRequiredSize, 0))
			{
...
{
   HANDLE h = FileCreate(m_ConnectedDevice.c_str(),GENERIC_READ,0,0,OPEN_EXISTING,FILE_FLAG_OVERLAPPED,0);
   if(h != INVALID_HANDLE_VALUE)
   {
		CloseHandle(h);
		return 1;
